...
# ------ ARGS -------------------------------------------------------
parser = argparse.ArgumentParser(description='run image-only models')
...
parser.add_argument('--scheduler', type=str, default='warmup_linear', choices=['linear', 'cosine', 'warmup_linear', 'onecycle', 'none'], help='learning rate scheduler type')
...
def main():
    ...
    model_path = None
    if args.save_model:
        # suffix _20 if using 20% subset
        from config import PATH as CFG_PATH, IMG_FMT as CFG_IMG_FMT
        suffix = "_20" if ("mvsa_20" in CFG_PATH[3] or "MVSA-Single-20" in CFG_IMG_FMT[3]) else ""
        model_path = results_dir+"MVSA_img{}__{}_task{}_seed{}_net.pth".format(
            suffix, args.model_name,args.task,args.seed) 
    
    logger.info("Training")
    val_filename = results_dir + "MVSA_img{}__{}_task{}_seed{}_metrics_val.csv".format(
        suffix, args.model_name,args.task,args.seed)
    te_filename =  results_dir + "MVSA_img{}__{}_task{}_seed{}_metrics_test.csv".format(
        suffix, args.model_name,args.task,args.seed)      
    image_model.train(
        train_loader,
        val_loader,
    ...
    if args.save_preds:
        ...
        preds_filename = results_dir+"MVSA_img{}__{}_task{}_seed{}_preds.csv".format(
            suffix, args.model_name,args.task,args.seed)
