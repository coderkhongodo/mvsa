...
parser.add_argument('--weight_decay', type=float, default=0.001, help='weight decay param')
parser.add_argument('--lr', type=float, default=5e-6, help='learning rate param')
parser.add_argument('--dropout', type=float, default=0.1, help='dropout param')
parser.add_argument('--warmup_steps', type=int, default=100, help='warmup steps for learning rate scheduler')
parser.add_argument('--gradient_clip', type=float, default=0.5, help='gradient clipping value')
parser.add_argument('--scheduler', type=str, default='warmup_linear', choices=['linear', 'cosine', 'warmup_cosine', 'warmup_linear', 'onecycle', 'none'], help='learning rate scheduler type')
...
def main():
    ...
    # decide suffix based on config (20% subset if using MVSA-Single-20 images)
    suffix = "_20" if ("MVSA-Single-20" in str(cfg.img_fmt)) else ""

    model_path = None
    if args.save_model:
        model_path = results_dir+"MVSA_txt{}__{}_task{}_seed{}_net.pth".format(
            suffix, args.model_name,args.task,args.seed) 
    logger.info("Training")        
    val_filename = results_dir + "MVSA_txt{}__{}_task{}_seed{}_metrics_val.csv".format(
        suffix, args.model_name,args.task,args.seed)
    te_filename =  results_dir + "MVSA_txt{}__{}_task{}_seed{}_metrics_test.csv".format(
        suffix, args.model_name,args.task,args.seed)  
    ...
    te_filename=te_filename, warmup_steps=args.warmup_steps, gradient_clip=args.gradient_clip,
    ...
    if args.save_preds:
        ...
        preds_filename = "MVSA_txt{}__{}_task{}_seed{}_preds.csv".format(
                suffix, args.model_name,args.task,args.seed)
    ...
    if args.eval_txt_test:
        ...
        preds_filename = "MVSA_txt{}__{}_task{}_seed{}_preds_txt.csv".format(
                suffix, args.model_name,args.task,args.seed)
        ...
        res_filename = "MVSA_txt{}__{}_task{}_seed{}_metrics_txt.csv".format(
                suffix, args.model_name,args.task,args.seed)
